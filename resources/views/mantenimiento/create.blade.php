...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-800">Nueva Orden de Trabajo</h1>
    <p class="text-sm text-gray-500 mt-1">Complete los campos para registrar un trabajo de mantenimiento.</p>
</div>
...
@csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="descripcion" rows="3"
                          ...
                          placeholder="Describa la falla o actividad realizada...">{{ old('descripcion') }}</textarea>
            </div>
    ...
    <div class="bg-white rounded-xl shadow p-6">
            ...
            @foreach($personals as $p)
                <label class="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                    <input type="checkbox" name="personal[]" value="{{ $p->id }}"
                           {{ in_array($p->id, old('personal', [])) ? 'checked' : '' }}
        ...
        @error('personal') <p class="text-red-500 text-xs mt-2">{{ $message }}</p> @enderror
    </div>
    ...
    <div class="bg-white rounded-xl shadow p-6">
        ...
        <p id="repuestos-vacio" class="text-sm text-gray-400 py-2">No hay repuestos agregados.</p>
    </div>
...
function autocompletarCosto(select, idx) {
    ...
    document.getElementById(`costo-${idx}`).value = costo;
}
